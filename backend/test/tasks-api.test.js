...
test("create, then PUT with an older updatedAt keeps server version", async () => {
  ...
  const created = await request(app, "POST", "/api/tasks", { body: task, headers: { "Idempotency-Key": "k1" } });
...
test("PUT with a newer updatedAt wins as client_won", async () => {
  ...
  await request(app, "POST", "/api/tasks", { body: task, headers: { "Idempotency-Key": "k3" } });
...
test("delete is a soft delete with a tombstone timestamp", async () => {
  ...
  await request(app, "POST", "/api/tasks", { body: task, headers: { "Idempotency-Key": "k5" } });
...
test("GET /api/tasks?since filters to tasks updated after the given time", async () => {
  ...
  const res = await request(app, "GET", "/api/tasks?since=2026-01-02T00:00:00.000Z");
